...
function QuoteDetailsAccordion({
  ...
  const handleRatingChange = async (
    ...
    try {
      const response = await updateAssetQuoteRating(
        taskId,
        quoteId,
        rating,
      );
      ...
      setQuotes((previous) =>
        previous.map((quote) =>
          quote.quote_id === quoteId
            ? {
                ...quote,
                executive_rating: response.quote.rating,
              }
            : quote,
        ),
      );
  ...
  const handleSubmitRatings = async () => {
    ...
    try {
      ...
      const unratedQuotes = quotes.filter((quote) => {
      ...
      const response = await submitAssetPurchaseRatings(
        taskId,
        ratings,
      );
      ...
      setSubmitSuccess(
        response.message ||
          "Quotation ratings submitted successfully.",
      );
...
function formatQuoteAmount(amount: number) {
